        ...
        onCommand={(cmd) => {
          switch (cmd.action) {
            ...
            case 'setIcon':
              ...
              break
            case 'appendToNote':
              if (cmd.content && editorApi) {
                import('marked').then(({ marked }) =>
                  marked.parse(cmd.content)
                ).then(html => {
                  const editor = editorApi.getEditor()
                  editor.chain().focus().insertContent('\n' + html).run()
                  const htmlContent = editorApi.getHTML()
                  const jsonContent = editorApi.getJSON()
                  const plainText = editorApi.getText()
                  note.html = htmlContent
                  note.content = JSON.stringify(jsonContent)
                  note.text = plainText
                  updateCounts(plainText)
                  queueSave()
                })
